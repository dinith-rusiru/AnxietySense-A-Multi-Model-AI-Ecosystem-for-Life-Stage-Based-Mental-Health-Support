...
export default function RelaxScreen({ route }) {
  ...
  return (
    <View style={styles.container}>
      ...
      {teenageYearRange ? (
        <Text style={styles.rangeText}>Teenage year range: {teenageYearRange}</Text>
      ) : null}

      {recommendedSongs.length === 0 ? (
        <View style={styles.emptyWrap}>
          <Text style={styles.emptyText}>No song recommendations available.</Text>
          <Text style={styles.emptySubtext}>Take a photo to get personalized song suggestions.</Text>
        </View>
      ) : (
